...
ls.stdout.on("data", data => {
  ...
  if (data.indexOf("Ingrese la url del servicio RESTful a construir las vistas crud") > 0 && expectedAnswers === 0) {
    ls.stdin.write("\n");
    expectedAnswers++;
  }
  if (data.indexOf("El servicio requiere cabecera de autencicación (JWT, Basic, etc.)") > 0  && expectedAnswers === 1) {
    ls.stdin.write("\n");
    expectedAnswers++;
  }
  if (data.indexOf("Ingrese la cabecera de autorización del servicio") > 0 && expectedAnswers === 2) {
    ls.stdin.write("\n");
    expectedAnswers++;
  }
  if (data.indexOf("Ingrese el nombre del servicio") > 0 && expectedAnswers === 3) {
    ls.stdin.write("\n");
    expectedAnswers++;
  }
